...
class OllamaClient:
    ...
    def generate(self, prompt, context: str = None, check_tokens: bool = True, model: str = None, options: dict = None):
        """
        ...
        Args:
            ...
            options: LLM options like {"temperature": 0}
        ...
        if self._use_llm_client:
            return self._client.generate(prompt, context=context, check_tokens=check_tokens, options=options)
        ...
        response = self._ollama_client.generate(model=use_model, prompt=prompt, options=options)

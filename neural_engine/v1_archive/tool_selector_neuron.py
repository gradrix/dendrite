...
class ToolSelectorNeuron(BaseNeuron):
    ...
    @classmethod
    def from_config(cls, config: 'SystemConfig') -> 'ToolSelectorNeuron':
        """
        Create ToolSelectorNeuron from a SystemConfig.
    ...
    def process(self, goal_id: str, goal: str, depth: int):
        ...
        if self.tool_discovery:
            ...
            # Core system tools that should ALWAYS be available
            CORE_TOOLS = {'memory_read', 'memory_write', 'python_script', 'generic_request'}
            
            # Build tool list: core tools first, then others up to limit
            tool_definitions = {}
            for name in CORE_TOOLS:
                if name in all_tools:
                    tool_definitions[name] = all_tools[name]
            
            # Add remaining tools up to limit of 10
            remaining_slots = 10 - len(tool_definitions)
            for name, tool_def in all_tools.items():
                if name not in CORE_TOOLS and remaining_slots > 0:
